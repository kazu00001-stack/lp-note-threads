...
async function callGemini(apiKey, { systemPrompt, userPrompt, jsonMode, maxOutputTokens }) {
...
async function researchLp(apiKey, lpText, analysis) {
  const system = `あなたはリサーチャーです。LP紹介記事のため、関連トピックのよくある疑問・比較ポイント・読者の不安を整理します。`;
  const user = `次のLPについて、紹介記事に使えるリサーチメモを400〜700字で書いてください。
箇条書き可。LPに書いていないことは推測と明示。
...
${lpSnippet(lpText, 6000)}`;
  ...
  return callGemini(apiKey, { systemPrompt: system, userPrompt: user, maxOutputTokens: 2048 });
}
...
export async function runGenerate({ apiKey, mode, lpText, analysis, research, allowNoResearch, options = {} }) {
  if (!apiKey?.trim()) throw new Error("Gemini APIキーを入力してください。");
  if (!lpText?.trim()) throw new Error("LPの全文を貼り付けてください。");

  const meta = { ...buildMeta(options), ...options };

  if (mode === "prep") {
    let parsedAnalysis = analysis;
    if (!parsedAnalysis) parsedAnalysis = await analyzeLp(apiKey, lpText);
    ...
    return { analysis: parsedAnalysis, research: researchText, researchFailed };
  }
  ...
  if (mode === "angles") {
    ...
    return { analysis: parsedAnalysis, result: angles };
  }
  ...
  if (mode === "note") {
    return { result: await generateNote(apiKey, lpText, parsedAnalysis, researchText, opts) };
  }
  if (mode === "threads") {
    return { result: await generateThreads(apiKey, lpText, parsedAnalysis, researchText, opts, 5, false) };
  }
  if (mode === "threads-pin") {
    return { result: await generateThreads(apiKey, lpText, parsedAnalysis, researchText, opts, 3, true) };
  }
